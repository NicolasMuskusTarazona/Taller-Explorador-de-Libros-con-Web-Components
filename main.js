document.addEventListener('DOMContentLoaded', () => {
    ...
    // Definir los datos de los libros (bookData)
    let bookData = [
        {
    ...
    // Generamos las Book Cards basados en el objeto bookData
    bookData.forEach(book => {
        const bookCard = document.createElement('book-card');
        bookCard.setAttribute('coverUrl', book.coverUrl);
        bookCard.setAttribute('title', book.title);
        bookCard.setAttribute('author', book.author);
        bookCard.setAttribute('synopsis', book.synopsis);
        bookCard.setAttribute('publishedDate', book.publishedDate);
        bookCard.setAttribute('genre', book.genre);
        bookCard.setAttribute('summary', book.summary);
        bookCard.setAttribute('quotes', JSON.stringify(book.quotes));

        // Escuchar el evento showDetail para mostrar los detalles
        bookCard.addEventListener('showDetail', () => {
            const bookDetail = document.createElement('book-detail');
            bookDetail.bookData = book; // Pasar todo el objeto de datos del libro

            booksContainer.innerHTML = ''; // Limpiar la vista anterior
            booksContainer.appendChild(bookDetail);

            // Agregar el evento para volver a la tarjeta
            bookDetail.addEventListener('backToCard', () => {
                booksContainer.innerHTML = ''; // Limpiar el detalle
